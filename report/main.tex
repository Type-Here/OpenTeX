...
Il sistema è interamente containerizzato; l'unico prerequisito è Docker ($\geq 24.0$) e Docker Compose ($\geq 2.20$).
...
\textbf{S}oft state & Gli aggregati derivati (es.\ conteggio collaboratori) sono calcolati on-demand, non in cache \\
\textbf{E}ventually \newline consistent & I log di attività e le modifiche ai permessi si propagano alla lettura successiva; nessuna sincronizzazione write-ahead \\
\bottomrule
\end{tabular}
...
\texttt{projects} & POST & \texttt{/projects/} & Creazione progetto \\
 & GET & \texttt{/projects/} & Elenco progetti (filtri \texttt{owner\_id}, \texttt{member\_id}; ricerca full-text \texttt{q}) \\
...
\bottomrule
\end{tabular}
\caption{Indici del sistema. I primi cinque sono creati da \texttt{scripts/indexes/create\_indexes.py} e verificati con \texttt{explain()}. Due di essi sono inoltre garantiti all'avvio dell'applicazione (\texttt{backend/app/main.py}). Il primo è un indice univoco su \texttt{users.email}, che assolve una duplice funzione: accelera la ricerca dell'utente in fase di login e impone a livello di database l'unicità dell'indirizzo email in fase di registrazione. Il secondo è \texttt{projects\_text\_search}, richiesto dalla ricerca della dashboard: in sua assenza MongoDB rifiuta la query \texttt{\$text} con errore \texttt{IndexNotFound}, condizione che si presenta dopo ogni esecuzione del seed con \texttt{--drop}, che elimina le collezioni e con esse i loro indici.}
\end{table}
...
\subsection{Uso applicativo dell'indice full-text}

L'indice \texttt{projects\_text\_search} non è un artefatto del solo benchmark: alimenta il campo di ricerca della dashboard, che invoca \texttt{GET /projects/?q=<termini>} eventualmente combinato con \texttt{owner\_id} o \texttt{member\_id} per restringere la ricerca ai progetti posseduti o condivisi. Il filtro risultante è una query \texttt{\$text} ordinata per rilevanza (\texttt{\{\$meta: "textScore"\}}). Il piano di esecuzione conferma l'uso dell'indice:

\begin{lstlisting}
FETCH -> TEXT_MATCH -> FETCH -> IXSCAN   (indexName: projects_text_search)
\end{lstlisting}

Si osservi che il filtro su \texttt{owner\_id} è applicato nello stadio \texttt{FETCH} esterno e non risolto dall'indice: MongoDB usa l'indice full-text per risolvere i termini e solo dopo scarta i documenti appartenenti ad altri proprietari. Un indice testuale composto con \texttt{owner\_id} come chiave prefisso risolverebbe entrambe le condizioni in un'unica scansione, ma MongoDB impone che ogni query su un indice testuale composto specifichi un'uguaglianza su tutte le chiavi prefisso: la ricerca globale, non vincolata a un proprietario, diverrebbe ineseguibile. La scelta dell'indice semplice è quindi un compromesso a favore della generalità della query.

Il limite semantico è che \texttt{\$text} confronta parole intere (con stemming), non prefissi: \texttt{quantum} trova ``Quantum computing'', \texttt{quant} no. Una ricerca per sottostringa richiederebbe un'espressione regolare non ancorata, che degenera in \texttt{COLLSCAN}, esattamente ciò che l'indice serve a evitare, e infatti la stessa equivalenza adottata come baseline ``senza indice'' in Tabella~\ref{tab:bench}.
...
\begin{itemize}[leftmargin=2.2em]
  ...
  \item \textbf{Documentazione} (Issue \#10): LLM usato per la generazione della documentazione, il fix del Dockerfile (build-arg \texttt{TARGETARCH}), le traduzioni e il polish del frontend.
\end{itemize}
