...
Login di test (creato dal seed): \texttt{admin@opentex.org} / \texttt{admin1234} (account amministratore di sito, con accesso a statistiche e benchmark).
...
\bottomrule
\end{tabular}
\caption{Endpoint REST implementati (15 endpoint su 7 router).}
\end{table}
...
% =====================================================
\section{ETL -- Generazione di dati sintetici}
% =====================================================
...
\bottomrule
\end{tabular}
\caption{Indici creati da \texttt{scripts/indexes/create\_indexes.py}, verificati con \texttt{explain()}.}
\end{table}
...
In conformità con i requisiti del corso, l'uso di strumenti di intelligenza artificiale generativa è dichiarato in modo trasparente nei form di valutazione associati a ciascuna issue (cartella \texttt{10/Form/}):

\begin{itemize}[leftmargin=2.2em]
  \item \textbf{Issue \#2} (generazione dati sintetici) -- assistente Claude usato come supporto alla generazione dello script di seed; output verificato ed eseguito prima del merge.
  \item \textbf{Issue \#10} (documentazione finale) -- Claude Code usato per la generazione della documentazione, il fix del Dockerfile (build-arg \texttt{TARGETARCH}), le traduzioni e il polish del frontend; output verificato prima del merge.
\end{itemize}

In tutti i casi il codice e i contenuti generati sono stati revisionati da un membro del team prima dell'integrazione, secondo il workflow standard: 1 issue $\to$ 1 branch $\to$ 1 PR $\to$ 1 review $\to$ merge.
...
Il progetto OpenTeX è stato sviluppato attraverso 10 issue, ciascuna chiusa con una pull request dedicata:

\begin{enumerate}[leftmargin=2.2em]
  \item Schema e validation delle 5 collezioni MongoDB
  \item Generatore di dati sintetici (seed)
  \item Setup repository, Docker Compose e configurazione base
  \item API CRUD per la gestione dei progetti
  \item Sistema di permessi e gestione collaboratori
  \item UI Dashboard "I miei Progetti"
  \item Aggregation pipeline per statistiche (JOIN tra collezioni)
  \item Creazione e ottimizzazione indici
  \item Benchmark query con e senza indici
  \item Documentazione finale e proof of concept
\end{enumerate}

Tutte le issue risultano chiuse e le relative pull request mergiate su \texttt{main} (l'ultima, PR \#28, relativa alla documentazione finale). Il sistema è interamente riproducibile da zero seguendo le istruzioni del README, non contiene credenziali reali nel repository, ed è stato verificato end-to-end (registrazione, login, creazione progetto, upload/editing file, compilazione LaTeX, gestione permessi, consultazione statistiche).
